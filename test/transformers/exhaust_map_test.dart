...
void main() {
  group('ExhaustMap', () {
    test('does not create a new Stream while emitting', () async {
      var calls = 0;
      final observable = RangeStream(0, 9).exhaustMap((i) {
        calls++;
        return TimerStream(i, Duration(milliseconds: 100));
      });
    ...
    test('is reusable', () async {
      final transformer = ExhaustMapStreamTransformer(
          (int i) => TimerStream(i, Duration(milliseconds: 100)));

      await expectLater(
        RangeStream(0, 9).transform(transformer),
    ...
    test('works as a broadcast stream', () async {
      final stream = RangeStream(0, 9)
          .asBroadcastStream()
          .exhaustMap((i) => TimerStream(i, Duration(milliseconds: 100)));
    ...
    test('can be paused and resumed', () async {
      ...
      final stream = RangeStream(0, 9)
          .exhaustMap((i) => TimerStream(i, Duration(milliseconds: 20)));
